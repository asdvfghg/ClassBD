...
from utils.DatasetLoader import CustomTensorDataset, NoisyDataset
            ...
            if phase == 'validation':
                ...
                if acc > max_acc:
                    ...
                    if not os.path.exists("Models"):
                        os.mkdir('Models')
                    # 存储模型参数
                    torch.save(net.state_dict(), f'Models/JNU_best_checkpoint_{config.chosen_model}.pth')
                    print("save model")
...
def inference(dataloader, chosen_model):
    ...
    state_dict = torch.load(f'Models/JNU_best_checkpoint_{chosen_model}.pth')
...
def main(config):
    ...
    if config.path == "Paderborn":
        Train_X, Val_X, Test_X, Train_Y, Val_Y, Test_Y = Paderborn_Processing(file_path=os.path.join('data', config.path), load=config.chosen_dataset, noise=config.add_noise, snr=config.snr, sample_number=config.number)
        config.class_num = 14
...
if __name__ == '__main__':
    ...
    # noisy condition6
    config.add_noise = 'Gaussian' # airplane, truck, Gaussian
    config.snr = -4

    # dataset and model
    config.path = 'Paderborn' # Paderborn JNU
    config.chosen_dataset = 'N09_M07_F10' # N09_M07_F10; N15_M01_F10; N15_M07_F04; N15_M07_F10; '20'; '30'
