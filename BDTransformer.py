...
class DSCTransformer(nn.Module):
    def __init__(self, *, input_dim=2048, dim=64, depth=4, heads=4, mlp_dim=256, pool='cls',
                 num_classes=10, channels=1, dim_head=64, emb_dropout=0.5, dropout=0.5):
    ...
    def forward(self, x):
        ...
        TimeFeature = self.TimeTrans(x)
        y = self.mlp_head(TimeFeature), k, g
...
if __name__ == '__main__':
    ...
    d_ff = 256  # 全连接层维度
    head = 4  # 注意力头数
    from Transformer import DSCTransformer
    model = DSCTransformer(input_dim=input_dim, num_classes=14, dim=d_model, depth=N,
                           heads=head, mlp_dim=d_ff, dim_head=d_model)
    model.eval()
    # print(model)
